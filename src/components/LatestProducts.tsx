    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-col items-center">
          <div className="h-1 bg-gray-200 rounded overflow-hidden w-32">
            <div className="w-24 h-full bg-indigo-500"></div>
          </div>
          <h1 className="text-indigo-900 font-medium text-3xl mt-6 mb-12 text-center">
            Latest Products
          </h1>
        </div>
        <div className="flex flex-wrap sm:-m-4 -mx-4 -mb-10  -mt-4">
          {FeaturedProduct.map((product) => (
            <div key={product.id} className="p-4 md:w-1/3 sm:w-1/2 w-full mb-6">
              <div className="rounded-lg bg-[#F7F7F7] overflow-hidden p-6 px-8 transition-all transform hover:scale-105 hover:shadow-xl hover:translate-y-[-10px] duration-300">
                <Image alt={product.title} src={product.imgSrc} height={300} width={250} className="object-contain px-2" />
                {/* <div className='flex gap-2'> */}
                <div className=" flex space-x-10">
                  <h2 className="text-xl font-medium text-violet-900">{product.title}</h2>
                  <p className="text-base  text-gray-600">
                    {product.price} <span className="text-sm text-gray-400 line-through">{product.oldPrice}</span>
                  </p>
                {/* </div> */}
                </div>
              </div>
